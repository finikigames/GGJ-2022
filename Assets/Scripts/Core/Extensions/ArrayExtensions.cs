...
namespace Source.Scripts.Core.Extensions
{
    public static class ArrayExtensions
    {
            ...
            where T : unmanaged
        {
            ...
            readOnlyNativeWrappedData.Array = wrappedData.Array;
#if UNITY_EDITOR
        ...
        public static unsafe ReadOnlyNativeWrappedData<T> WrapToReadOnlyNative<T> (this T[] managedData) where T : unmanaged {
            fixed (void* ptr = managedData) {
                ...
                var sh = AtomicSafetyHandle.Create ();
                NativeArrayUnsafeUtility.SetAtomicSafetyHandle (ref nativeData, sh);
                return new ReadOnlyNativeWrappedData<T> { Array = nativeData, SafetyHandle = sh };
#else
                return new ReadOnlyNativeWrappedData<T> { Array = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T> (ptr, managedData.Length, Allocator.None) };
#endif
            }
        }
